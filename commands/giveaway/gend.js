...
module.exports = {
    ...
    categories: "Giveaway",
    example: 'gend 843810523891761222',
    usage: '<Message ID>',
...
module.exports.run = async(sime, message, args) => {
    ...
    if(giveaway.ended == true) {
        sime.giveaways.edit(giveaway.ended = false)
    }
    sime.giveaways.edit(giveaway.messageID, {
        setEndTimestamp: Date.now()
    }).then(() => {
        message.lineReply(':white_check_mark: Giveaway will end/reroll in less than 5 seconds...')
    }).catch(err => {
        console.log(err)
